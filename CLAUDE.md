...
- **`tools/` is not a build step.** `tools/build-writing.mjs` is a zero-dependency maintenance script run by hand to regenerate `writing.html` from `index.html`'s `WRITING` array. The site still deploys as plain static files with nothing to compile — that distinction is what keeps the rule above intact. Do not add a `package.json`, a watcher, or a CI hook that runs it.
